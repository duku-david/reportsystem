...
.container
{
  ...
  width: 98%;
    ...
    <?php
    $sql="SELECT * FROM student INNER JOIN marksheet ON student.Reg_no = marksheet.Reg_no WHERE c_id IN (SELECT c_id FROM class WHERE c_name='".$form."') AND stud_id='".$stud_id."' AND term_id='".$_SESSION['term_id']."'";
          $runst    =mysqli_query($conn,$sql) or die(mysqli_error($conn));

      $sqlc="SELECT COUNT('firstname') AS countf FROM student INNER JOIN marksheet ON student.Reg_no = marksheet.Reg_no WHERE c_id IN (SELECT c_id FROM class WHERE c_name='".$form."') AND term_id='".$_SESSION['term_id']."'";
          $runc   =mysqli_query($conn,$sqlc) or die(mysqli_error($conn));
