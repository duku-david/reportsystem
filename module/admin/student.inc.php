  ...
  if(!in_array($ext, $array))
   ...
   }else
   {
         ...
         $result=$resultm=$results="";
            ...
            $sqlsub = "SELECT sub_cod FROM subject";
           ...
           }else
              {
                ...
                //insert into marksheet table
                 for($k=0; $k < count($term_ids); $k++) 
                 {
                   ...
                   $sql = "INSERT INTO student(`firstname`, `othername`, `Reg_no`, `gender`, `picture`, `c_id`, `year`) VALUES('".$firstname."','".$othername."','".$Reg_no."','".$gender."','".$folder."','".$form."','".$year."')";
                   $result= mysqli_query($conn,$sql) or die(mysqli_error($conn));
