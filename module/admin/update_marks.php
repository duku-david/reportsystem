<?php
  ...
  ';
                 ...
                 $sql ="SELECT * FROM (grade INNER JOIN student ON grade.Reg_no = student.Reg_no) WHERE student.c_id IN(SELECT c_id FROM subjecttakenbyteacher WHERE initials ='".$initials."' AND sub_cod='".$subject."' AND c_id='".$c_id."') AND grade.sub_cod='".$subject."' AND term_id='".$_SESSION['term_id']."' AND year='".$_SESSION['year']."' ORDER BY firstname ";
...
if( $mid_term > 30 || $end_term > 70 || $total > 100){
    ...
    }else
    {
         ...
         $sql ="SELECT SUM(total) AS old_total FROM grade WHERE sub_cod !='".$sub_cod."'AND Reg_no='".$Reg_no."' AND term_id ='".$term_id."'";
          ...
          $query = mysqli_query($conn,"SELECT name FROM subject WHERE sub_cod='".$sub_cod."'");
          $sub_name='';
