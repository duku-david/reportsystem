...
<?php include('grade_action.php');?>
  ...
  <?php
       ...
       $queryt = "SELECT name FROM subject WHERE sub_cod IN(SELECT sub_cod FROM subjecttakenbyteacher WHERE initials='".$initials."' AND c_id='".$c_id."' AND year='".$_SESSION['year']."')";
                $run=mysqli_query($conn,$queryt) or die(mysqli_error($conn));
                $result = mysqli_fetch_array($run);
        if(mysqli_num_rows($run)==0)
        {
            echo'<div class="alert alert-danger" style="text-align:center;">
          <a href="index.php" style="float: left; margin-left: 20px; margin-top: 0px;" class="btn btn-primary">Back</a>
              <p>Mark sheet unavaliable</p>
          </div>';
        }else
        {
